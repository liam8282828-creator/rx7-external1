...
enum FileManagerService {
    ...
    // MARK: - Missing methods required by FileBrowserView

    @discardableResult
    static func renameItem(at sourceURL: URL, to newName: String) throws -> URL {
        let fileManager = FileManager.default
        guard fileManager.fileExists(atPath: sourceURL.path) else {
            throw FileManagerOperationError.sourceMissing
        }
        let validName = try validatedName(newName)
        let destination = sourceURL.deletingLastPathComponent().appendingPathComponent(validName)
    ...
    static func createZIPArchive(
        ...
        // ZIP archive creation is not supported in this build.
        throw FileManagerOperationError.cannotArchive
    }
    ...
    static func transferItem(
        ...
        if fileManager.fileExists(atPath: destinationURL.path) {
            ...
            case .keepBoth:
                destinationURL = uniqueDestinationURL(
                    for: destinationURL,
                    isDirectory: (try? destinationURL.resourceValues(forKeys: [.isDirectoryKey]).isDirectory) ?? false,
                    fileManager: fileManager
                )
            }
        }
        ...
        do {
            ...
            throw mode == .copy ? FileManagerOperationError.cannotCopy : FileManagerOperationError.cannotMove
        }
        return FileTransferResult(sourceURL: sourceURL, destinationURL: destinationURL, disposition: disposition)
    }
}
